...
def engine_factory(api_key=None, model=None, **kwargs):
    ...
    if model in ["gpt-4o-mini", "gpt-4o"]:
        ...
        # Initialize the Llama 3.2 Vision engine
        return LlamaBedrockEngine(model=model, **kwargs)
    else:
        raise Exception(f"Unsupported model: {model}, currently supported models: \
                        gpt-4-vision-preview, gpt-4-turbo, gpt-4o, gemini-1.5-pro-latest, llava, llama-3.2-vision")
...
class LlamaBedrockEngine(Engine):
    def __init__(self, **kwargs):
        ...
        self.model = "bedrock/us.meta.llama3-2-11b-instruct-v1:0"  # Replace with the correct model identifier
    ...
    def generate(self, prompt: list = None, max_new_tokens=4096, temperature=None, model=None, image_path=None,
                 output_0=None, turn_number=0, **kwargs):
        ...
        if turn_number == 0:
            ...
            if base64_image:
                messages[-1]["images"] = [base64_image]
            messages.append({"role": "assistant", "content": output_0})
            messages.append({"role": "user", "content": prompt2})

        # Use litellm to call the model via AWS Bedrock
        response = litellm.completion(
            model=model if model else self.model,
            messages=messages,
            max_tokens=max_new_tokens if max_new_tokens else 4096,
